#!/usr/bin/env bash
# guard-destructive-bash.sh — DocShell 的 PreToolUse 安全护栏
#
# 作用:在 --permission-mode bypassPermissions(全自动放行工具调用)下,仍然
...
#   和"一句话诱导模型删家目录",不防有动机的攻击者。要真正隔离,请用容器 / 独立受限
...
#   删除目标恰是根 / 家 / 一级系统目录时才拦。
#
# 装载:服务启动时通过 `--settings` inline JSON 注入,指向本脚本绝对路径(见 lib/cc-process.ts
#   的 buildSettings)。
set -uo pipefail
