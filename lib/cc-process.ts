...
const MAX_PROCESSES = parseMaxProcesses(process.env.DOCSHELL_MAX_PROCESSES);
...
interface DocProcess {
  ...
  busy: boolean;                         // 正在向前端流式输出；容量回收绝不终止活跃回合
  lastUsedAt: number;                    // 容量满时只回收最久未使用的空闲进程
}
...
function makeProc(args: string[], sessionId: string, effort: string): DocProcess {
  ...
  const dp: DocProcess = {
    ...
    busy: false, lastUsedAt: Date.now(),
  };
...
function stopProcess(dp: DocProcess, signal: NodeJS.Signals = 'SIGTERM') {
  if (dp.idleTimer) { clearTimeout(dp.idleTimer); dp.idleTimer = null; }
  try { dp.proc.kill(signal); } catch { /* noop */ }
}

// 为真实回合腾位置时，只回收最久未使用的空闲进程。全部都在流式输出则拒绝新回合，
// 不能为了容量上限中断另一份文档。预热不触发回收，它只是性能优化。
function reserveProcessSlot(evictIdle: boolean): boolean {
  if (procs.size < MAX_PROCESSES) return true;
  if (!evictIdle) return false;
  const key = selectIdleProcessForEviction(procs.entries());
  if (!key) return false;
  const dp = procs.get(key);
  if (!dp) return false;
  procs.delete(key);
  stopProcess(dp);
  return true;
...
function registerProcess(
  ...
  if (!reserveProcessSlot(true)) {
    throw new Error(`DocShell process capacity reached (${MAX_PROCESSES} active conversations)`);
  }
  const dp = makeProc(args, sessionId, effort);
...
async function doTurn(opts: RunTurnOpts): Promise<{ sessionId: string }> {
  ...
  let sessionToResume = resumeSessionId;
  if (existing?.alive) {
    if (existing.effort === effort) return turnOn(existing, prompt, onChunk, false, signal);
    sessionToResume = sessionIdForRestart(existing.sessionId, resumeSessionId);
  ...
  if (sessionToResume) {
    const dp = registerProcess(
      conversationId,
      ['--resume', sessionToResume, ...baseArgs(model, effort)],
      sessionToResume,
      effort,
    );
    try {
  ...
  const fresh = registerProcess(conversationId, baseArgs(model, effort), '', effort);
  // 会话过期重开 → 先在正文里诚实示意（走同一 onChunk，排在回复文本之前）
  if (sessionToResume) onChunk({ kind: 'text_delta', text: '（上次会话已过期，已为你重新开始；之前的上下文我看不到了）\n\n' });
...
export function warmUp(conversationId: string, model = 'opus', effort = 'max', resumeSessionId?: string): void {
  ...
  if (!reserveProcessSlot(false)) return;         // 预热不能挤掉真实会话，也不能突破容量上限
...
export function shutdownAllProcesses(): void {
  for (const dp of procs.values()) stopProcess(dp, 'SIGKILL');
  procs.clear();
}
